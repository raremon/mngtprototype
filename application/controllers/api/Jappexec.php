...
class Jappexec extends REST_Controller {
	
	public function __construct() {
        parent::__construct();
		$this->load->model('appexec_model','Appexec');
    }
...
`tbappexec` (
  ...
  Posted variables: app, filename, route
		*/
			
		if( isset($d['app']) && isset($d['filename']) && isset($d['route']) ){

			//check if appexec already exists
				
			$where = array(
							'Appid'=>$d['app'],'RouteId'=>$d['route'],'AppFilename'=>$d['filename']
							);
				
			$check = $this->Appexec->getAppexec($where);
			...
			if( count($check)>0 ){ //update old update with new update
				...
				$result = $this->Appexec->update_Appexec($record,$where);
				...
				$result = $this->Appexec->save_Appexec($record);		
