...
class Jappdb extends REST_Controller {
	
	public function __construct() {
        parent::__construct();
		$this->load->model('appdb_model','Appdb');
    }
		
	public function index_get() {
		
		/* JSON method to return updates for the infotainment app/exe */
		//http://localhost/mngtprototype/api/jappdb/get/app/001
		//http://180.232.67.229 
		...
		if( isset($data['app']) ){
			$where = array('Appid'=>$data['app']);
			$response = $this->Appdb->getAppdb($where);
		}
		else{			
	...
	public function addappdb_post(){
		
		$d = $this->post();

		/* JSON method to save app db updates via POST method call by VB app */
		// http://[::1]/mngtprototype/api/jappdb/addappdb
		/* POSTED variables:	app, sqlcom, dbcom, tbname 
		...
		*/
		...
		if( isset($d['app']) && isset($d['sqlcom']) && isset($d['dbcom']) && isset($d['tbname']) ){

			//check if app db already exists
				
			$where = array(
							'Appid'=>$d['app'],'SqlCom'=>$d['sqlcom'],'DbCom'=>$d['dbcom'],'TbName'=>$d['tbname']
							);
				
			$check = $this->Appdb->getAppdb($where);
			...
			if( count($check)>0 ){ //update old update with new update
				...
				$result = $this->Appdb->update_Appdb($record,$where);
				...
				$result = $this->Appdb->save_Appdb($record);		
