...
class Jschedule extends MY_Controller {
	...
	public function getSchedule($busID=null,$today=null){
		
		/* JSON method to return list of ads to be played based on busID within routeID and airing date */
		
		if( isset($busID) && isset($today) && is_numeric($busID) && $today!='' ){
			
			// echo json_encode($busID.'.'.$today);
			
			$this->load->model('schedule_model');
			
			$schedule = $this->schedule_model->getSchedule($busID,$today);
	...
	public function getScheduleAds($busID=null,$today=null){
		...
		if( isset($busID) && isset($today) && is_numeric($busID) && $today!='' ){
			...
			$schedule = $this->schedule_model->getScheduleAds($busID,$today);
