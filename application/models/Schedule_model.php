...
class Schedule_model extends CI_Model
{
	...
	public function getSchedule($routeID,$date){
		...
		$this->db->select('*')
				...
				->join('airtimes','schedule.schedule_id=airtime.schedule_id','inner')
		...
		// if( isset($where) )
			$this->db->where('schedules.route_id',$busID);
	...
	// WHERE buses.bus_id=1 AND schedule.date_start<'2017-04-27' AND schedule.date_end>'2017-04-27' 
		...
		$this->db->select('*')
				...
				->join('airtimes','schedules.schedule_id=airtime.schedule_id','inner')
				// ->join('buses','routes.route_id=buses.route_id','inner')
				->join('ad_sched','schedules.schedule_id=ad_sched.schedule_id','inner')
